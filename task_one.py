...
def generate_random_number(n: int = 15) -> list:
    i = 1
    result = []
    while i < n:
...
def main():
    ...
    parser = argparse.ArgumentParser(prog="starwarAPI",
                                     usage="fetches resources from 'swapi.dev' "
                                           "based on whatever argument we provide",
                                     description="Its uses random number generator and uses"
                                                 " requests library to get value from swapi.dev")

    # we are creating an option to provide count

    parser.add_argument('-c', "--count", default=5, help="count of characters to fetch data from")
    parser.add_argument('-s', "--start", default=1, help="start of the range")
    parser.add_argument('-e', "--end", default=83, help="end of the range")
    parser.add_argument("-r", "--resource", default="people", help="name of resources",
    ...
    print(f"[INFO] produced {len(resources)} random resource id in range (1,83).")

    data = []
    for resource_id in resources:
        print(f"[INFO] fetching data for resource id {resource_id}...")
